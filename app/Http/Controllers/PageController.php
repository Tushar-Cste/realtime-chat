...
class PageController extends Controller
{
    ...
    {
        $data['res'] = User::where('email','=',$_POST['email'])->get()->toArray();
        echo json_encode($data);
    }
    public function wallet(Request $request)
    {
        $id              = $request->user()->id;
        $orders          = Order::all();
        $pending_transactions    = Balance::where('datwise',"<",date("Y-m-d"))->where('user_id','=',$id)->get()->toArray();
        $posted_transactions    = Balance::where('datwise',"=",date("Y-m-d"))->where('user_id','=',$id)->get()->toArray();
        // dd($transactions);
        // $daily_transactions_result  = $this->dailytransactionCalculator($transactions);
        $total_balance_remained    = $this->calculateTotalBalance($id);
        return view('pages.wallet.index', array('user' => Auth::User(),'orders'=>$orders , 'pending_transactions' => $pending_transactions , 'posted_transactions' => $posted_transactions ,'total_balance_remained' => $total_balance_remained));
    }
    public function calculateTotalBalance($id)
    {
        $credit_sum = 0;
        $debit_sum  = 0;
        $transactions    = Balance::where('user_id','=',$id)->get()->toArray();
    ...
    public function membership()
    {
        ...
        // return view('pages.admin.membership',['countries'=>$countries,'isAdmin'=>$isAdmin]);
        $isAdmin = 1;
        return view('pages.admin.premembership',['isAdmin'=>$isAdmin,'professions'=>$professions]);
    }
    ...
    public function userMembershipdetails()
    {
        $countries = array();
        ...
        $user = Auth::User();
        $userId = $user->id;
        $professions = Profession::get(['profession_type']);
        $professions_types = $professions->unique('profession_type')->toArray();
        $professions_name = Profession::where('profession_type','=','Doctor')->get(['profession_name'])->toArray();
        $isAdmin = DB::table('user_menu')->where('user_id',$userId)->where('menu_options_id',14)->get()->count();
        return view('pages.admin.membership',['countries'=>$countries,'isAdmin'=>$isAdmin,'professions_types'=>$professions_types,'professions_name'=>$professions_name]);
    }

    public function getProfessionOfType()
    {
        $profession_type = $_POST['profession_type'];
        $professions_name = Profession::where('profession_type','=',$profession_type)->get(['profession_name'])->toArray();
        $data['professions'] = $professions_name;
        echo json_encode($data);
    ...
    {
        $total_balance = $this -> calculateTotalBalance($id,$transfer_amount);
    ...
    public function getTransByDateRange(Request $request)
    {
        ...
        //dd($end_date);
        $transactions = Balance::where('datwise',"<=",$end_date)->where('datwise',">=",$start_date)->where('user_id','=',$id)->get()->toArray();
        ...
        foreach($transactions as $key=>$value)
        {
            if($value['datwise'] == date("Y-m-d"))
            {
                array_push($posted_trans,$value);
            }
            if($value['datwise'] != date("Y-m-d"))
            {
                array_push($pending_trans,$value);
            }
        }
        return view('partials.trans',array('posted_transactions' => $posted_trans ,'pending_transactions' => $pending_trans)) -> render();
